      ...
      <aside className={`fixed inset-y-0 left-0 z-40 w-64 transform border-r border-sidebar-border bg-sidebar transition-transform lg:static lg:translate-x-0 ${open ? "translate-x-0" : "-translate-x-full"}`}>
        ...
        <div className="absolute bottom-0 left-0 right-0 border-t border-sidebar-border p-3">
          <div className="flex items-center gap-3 rounded-lg bg-sidebar-accent/40 p-3">
            ...
            <button onClick={handleLogout} className="text-muted-foreground hover:text-destructive" title="Sign out">
        ...
        <header className="sticky top-0 z-30 flex h-16 items-center gap-4 border-b border-border bg-background/80 px-4 backdrop-blur-md md:px-6">
          ...
          <nav className="hidden items-center gap-1.5 text-sm text-muted-foreground md:flex">
            ...
            {crumbs.slice(1).map((c, i) => (
              <span key={i} className="flex items-center gap-1.5">
                ...
                <span className={i === crumbs.length - 2 ? "text-foreground font-medium capitalize" : "capitalize"}>{c.replace("-", " ")}</span>
              </span>
            ))}
          </nav>
          <div className="ml-auto flex items-center gap-2">
            <div className="relative hidden md:block">
              ...
              <input placeholder="Search tickets, projects, alerts..." className="w-72 rounded-lg border border-border bg-input/50 py-2 pl-9 pr-3 text-sm outline-none focus:border-primary" />
            </div>
            <button className="relative grid h-9 w-9 place-items-center rounded-lg border border-border bg-card hover:bg-accent" title="Notifications">
            ...
            <button className="grid h-9 w-9 place-items-center rounded-lg border border-border bg-card hover:bg-accent"><HelpCircle className="h-4 w-4" /></button>
